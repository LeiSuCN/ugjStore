...
<%
	...
	for( Store store: stores ){
		String detailUrl = request.getContextPath() + "/admin/store/detail/" + store.getId();
		if( rowIdx%2 == 0 ){
			out.print("<tr class='ui-state-highlight'>");
		} else{
			out.print("<tr>");
		}
		out.print("<td>" + store.getId() + "</td>");
		out.print("<td>" + store.getName() + "</td>");
		out.print("<td>" + store.getAddress() + "</td>");
		out.print("<td>" + store.getPhonenumber() + "</td>");
		out.print("<td>");
		out.print("<div class='hidden-sm hidden-xs action-buttons'>");
		out.print("<a class='blue' href='" + detailUrl + "' >");
		out.print("<i class='ace-icon fa fa-search-plus bigger-130'></i> </a> </div>");
  		out.print("</td>");
		out.print("</tr>");
		rowIdx++;
	}
