...
<div class="container">
	<form action="<%=root %>/contract/xiaobaitu/apply" method="post" enctype="multipart/form-data" onsubmit="return checkinput()" class="contract-content-container">
		<div>
			<label for="yifang">乙方</label>：<input name="yifang"  id="yifang"  type="text"  value='<%=contract.getYiFang() %>'  <%=isCanEdit %> />
		</div>
		<div>
			<label for="gongshangzhucehao">工商注册号</label>：<input name="gongshangzhucehao" type="text" value='<%=contract.getGongShangZhuCeHao() %>'  <%=isCanEdit %> />
		</div>
		<div>
			<label for="fadingdaibiaoren">法定代表人</label>：<input name="fadingdaibiaoren" type="text" value='<%=contract.getFaDingDaiBiaoRen() %>'  <%=isCanEdit %> />
		</div>
		<div>
			<label for="dianhua">电话</label>：<input name="dianhua" type="text"  value='<%=contract.getDianHua() %>'  <%=isCanEdit %> />
		</div>
		<div>
			<label for="dizhi">地址</label>：<input name="dizhi" type="text"  value='<%=contract.getDiZhi() %>'  <%=isCanEdit %> />
		</div>
		<div>
			<label for="youbian">邮编</label>：<input name="youbian" type="text"  value='<%=contract.getYouBian() %>'  <%=isCanEdit %> />
		</div>
		<div>
			...
			%>
				<input name="yingyezhizhao" type="file" />
			<%
				}
			%>
		</div>
