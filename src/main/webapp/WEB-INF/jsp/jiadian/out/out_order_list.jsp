                                    	...
                                    	<label for="orderStatus" class="label label-lg label-pink arrowed-right" style="float:left">订单状态</label>
 										<div class="col-xs-1">
											<select id="orderStatus" name="orderStatus" >
												<%
												out.print("<option value=" + OrderProcess.Status.ACCEPT.id + ">" + OrderProcess.Status.ACCEPT.name + "</option>");
												out.print("<option value=" + OrderProcess.Status.COMPLETE.id + ">" + OrderProcess.Status.COMPLETE.name + "</option>");
												%>
											</select>
                                    	</div>
                                   </div>
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        <div class="hidden-sm hidden-xs action-buttons">
                                                            <a class="blue" href="<c:url value="/out/order/xiaobaitu/detail/${order.id }" />">
...
<%@include file="/WEB-INF/jsp/common/out_footer_js.jsp" %>
        ...
        $.datepicker.setDefaults( $.datepicker.regional[ "zh-CN" ] );
        	...
        	function jump(){
        		...
        		var href = "?dateFrom=" + df + "&dateTo=" + dt + "&status=" + status + "&storeId=" + storeId;
            ...
            jQuery(function($) {
                ...
                .dataTable( {
                    ...
                    "oLanguage": {
               		...
               		"bProcessing": true,
        			"bServerSide": true,
                	"sAjaxSource": '<c:url value="/out/order/xiaobaitu/list/a?${requestScope.urlParams}"/>',
                 } );
            });
        </script>        
